...
export default function ManageMembers() {
  ...
  const categories = [
    {
      title: "1학년",
      members: viewData.filter((member) => member.grade === "1학년"),
    },
    {
      title: "2학년",
      members: viewData.filter((member) => member.grade === "2학년"),
    },
    {
      title: "3학년",
      members: viewData.filter((member) => member.grade === "3학년"),
    },
    {
      title: "4학년",
      members: viewData.filter((member) => member.grade === "4학년"),
    },
    {
      title: "초과학기",
      members: viewData.filter((member) => member.grade === "초과학기"),
    },
    {
      title: "휴학생",
      members: viewData.filter((member) => member.grade === "휴학"),
    },
    {
      title: "졸업생",
      members: viewData.filter((member) => member.grade === "졸업"),
    },
  ];
    ...
    <div className="font-pretendard">
      <div className="flex m-4 w-[1400px]">
        ...
        <div className="flex flex-row items-center gap-[15px] ml-auto">
          <Button className="ml-2" onClick={ChangeGradeAll}>
            전체 학년 증가
          </Button>
          <Button className="ml-2" onClick={DownloadExcel}>
            To Excel
          </Button>
          <Button className="ml-2" onClick={SaveChange}>
            변경 저장
          </Button>
          <Button className="ml-2" variant={"text"} onClick={CancelChange}>
            변경 취소
          </Button>
        </div>
      </div>
