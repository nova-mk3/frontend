  ...
  const handleRemoveFile = (index: number): void => {
    ...
    <div className='flex flex-col gap-2 my-5 t-m'>
        <div className="flex flex-row gap-2 items-center h-[30px] mb-2">
          ...
          <p className='text-success mt-auto l-l ml-3'> {selectedFiles.length} / 10</p>
        </div>
        <div className='flex flex-row flex-wrap gap-5'>

          {
            selectedFiles.length > 0 && selectedFiles.map((file, index) => (<FileItem name={file.file.name} onRemove={handleRemoveFile} index={index} key={index} preview={file.preview}/>))
          }
            <label htmlFor="fileUpload" className="inline-flex cursor-pointer items-center justify-center w-[100px] h-[100px] rounded-lg border-primary border-[3px] hover:scale-[1.03]"><FilePlus width={50} height={50}/></label>
            <input
            id="fileUpload"
            type="file"
            accept=".jpg, .jpeg, .png"
